...
function editor_html($id, $content, $is_dhtml_editor=true)
{
	...
	if ($is_dhtml_editor && $js) {
        $html .= "<script src=\"{$editor_url}/tinymce.min.js\"></script>";
		$html .= "<script src=\"{$editor_url}/custom.js\"></script>";
		
		$html .= "<script>var g5_editor_url = \"{$editor_url}\", oEditors = [], ed_nonce = \"'\.ft_nonce_create('tinymce').\'\";</script>";

		$js = false;
    }

    $tinymce_class = $is_dhtml_editor ? "tinymce" : "";
    $html .= "\n<textarea id=\"$id\" name=\"$id\" class=\"$tinymce_class\" maxlength=\"65536\" style=\"width:100%;height:200px\">$content</textarea>";
