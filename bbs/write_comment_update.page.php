...
if($comment_min > 0 || $comment_max > 0) {
    ...
    $comment_cnt = mb_strlen(apms_get_text($wr_content), 'UTF-8'); 
 
    if ($comment_min > 0 && $comment_min > $comment_cnt) {
        apms_alert('1|댓글은 이모티콘 등을 제외하고 '.$comment_min.'글자 이상 쓰셔야 합니다.');
    } else if ($comment_max > 0 && $comment_max < $comment_cnt) {
        apms_alert('1|댓글은 이모티콘 등을 제외하고 '.$comment_max.'글자 이하로 쓰셔야 합니다.');
    }
}
