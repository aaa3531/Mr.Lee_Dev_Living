...
function display_type($type, $list_skin='', $list_mod='', $list_row='', $img_width='', $img_height='', $ca_id='')
{
    ...
    if ($type == 1){
        $sql .= " order by it_1 desc limit $items "; // APMS : 노출순서 조정     - 2014.07.28
        $result = sql_query($sql);
    } else if($type == 2){
        $sql .= " order by it_2 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28 
        $result = sql_query($sql);
    } else if($type == 3){
        $sql .= " order by it_3 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28
        $result = sql_query($sql);
    } else if($type == 4){
        $sql .= " order by it_4 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28
        $result = sql_query($sql);
    } else if($type == 5){
        $sql .= " order by it_5 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28
        $result = sql_query($sql);
    } else if($type == 6){
        $sql .= " order by it_6 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28
        $result = sql_query($sql);
    } else if($type == 7){
        $sql .= " order by it_7 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28
        $result = sql_query($sql);
    } else if($type == 8){
        $sql .= " order by it_8 desc limit $items "; // APMS : 노출순서 조정 - 2014.07.28
        $result = sql_query($sql);
    }
    /*
