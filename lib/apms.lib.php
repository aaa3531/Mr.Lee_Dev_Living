...
// BS3 Style
function apms_paging($write_pages, $cur_page, $total_page, $url, $add='', $first='', $prev='<span>이전</span>', $next='<span>다음</span>', $last='') {
...
function apms_ajax_paging($id, $write_pages, $cur_page, $total_page, $url, $add='', $first='<span>이전</span>', $prev='', $next='', $last='<span>다음</span>') {
	...
	if ($start_page > 1) { 
		...
		$str .= '<li class="disabled" style="display:none"><a>'.$prev.'</a></li>'; 
	}
	...
	if ($total_page > $end_page) {
		...
		$str .= '<li class="disabled" style="display:none"><a>'.$next.'</a></li>';
	}
