...
// Intro
$intro_id = 'apms_shop_intro';
$intro_set = $default['as_intro'];
$intro_skin = $default['as_'.MOBILE_.'intro_skin'];
...
if($intro_type == "2") { // 지정날짜
	if((int)$intro_time > G5_SERVER_TIME) {
		...
		sql_query(" update `{$g5['g5_shop_default_table']}` set as_intro = '', as_intro_skin = '', as_mobile_intro_skin = '' ", false); // 인트로 해제
	}
...
if($is_intro) {
	if(is_file(G5_SKIN_PATH.'/intro/'.$intro_skin.'/intro.html')) {
		...
		$enter_url = (isset($default['de_root_index_use']) && $default['de_root_index_use']) ? G5_URL : G5_SHOP_URL;
