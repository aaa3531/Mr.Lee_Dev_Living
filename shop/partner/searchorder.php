...
//		if(!$_POST['od_id'][$i] || !$_POST['it_id'][$i]) continue;
		...
		if($_POST['pt_send'][$i] && $_POST['pt_send_num'][$i]) {
			$ct_status = '배송';
		} else if($_POST['ct_status'][$i] == '주문' ||$_POST['ct_status'][$i] == '입금' || $_POST['ct_status'][$i] == '준비' || $_POST['ct_status'][$i] == '배송') {
			$ct_status = $_POST['ct_status'][$i];
		} 
        
        $pt_send_date = G5_TIME_YMDHIS;

        $sql = "update {$g5['g5_shop_cart_table']}
                   set pt_send        = '".addslashes($_POST['pt_send'][$i])."',
                       pt_send_num    = '".addslashes($_POST['pt_send_num'][$i])."',
					   pt_datetime   = '$pt_send_date',
                       ct_status	  = '$ct_status'
                 where od_id = '{$_POST['od_id'][$i]}' and it_id = '{$_POST['it_id'][$i]}' and pt_id = '{$member['mb_id']}' and find_in_set(pt_it, '{$ct_it}')=0 and ct_select = '1' ";
		//echo $sql;
		sql_query($sql);

		// 주문 상품이 하나일 경우 / 주문 상품이 여러개라도 모두 배송처리 된 경우 오더테이블도 같이 업데이트
		$delivery_cnt_sql = sql_fetch(" select count(od_id) as d_cnt from {$g5['g5_shop_cart_table']} where od_id = '{$_POST['od_id'][$i]}' and ct_status='배송'");
		$if_sql = " select count(od_id) as cnt from {$g5['g5_shop_cart_table']} where od_id = '{$_POST['od_id'][$i]}'";
		$if_result = sql_fetch($if_sql);
		if ($if_result['cnt']==1) {
			$sql = " update {$g5['g5_shop_order_table']} 
						set od_delivery_company		= '".addslashes($_POST['pt_send'][$i])."', 
						od_invoice					= '".addslashes($_POST['pt_send_num'][$i])."', 
						od_invoice_time				= '$pt_send_date',
						od_status					= '$ct_status'
					where od_id = '{$_POST['od_id'][$i]}' ";
			sql_query($sql);
		} else if ($if_result['cnt'] == $delivery_cnt_sql['d_cnt']) {
			$sql = " update {$g5['g5_shop_order_table']} 
						set od_delivery_company		= '".addslashes($_POST['pt_send'][$i])."', 
						od_invoice					= '".addslashes($_POST['pt_send_num'][$i])."', 
						od_invoice_time				= '$pt_send_date',
...
// 상태
if(isset($done) && $done) { 
	$ct_status = ($done == "2") ? '배송' : '주문,입금,준비'; 
} else { 
	$ct_status = '주문,입금,준비,배송'; 
} 
...
//
//$sql_common = " from {$g5['g5_shop_cart_table']} where find_in_set(ct_status, '준비') and find_in_set(pt_it, '{$ct_it}')=0 and pt_id = '{$member['mb_id']}' and ct_select = '1' $sql_search and SUBSTRING(ct_select_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
...
if ($stx) {
	...
	$sql_search .= "and od_name = '%$stx%' ";
    if ($save_stx != $stx)
        $page = 1;
}
$sql_common = " from {$g5['g5_shop_order_table']} where find_in_set(od_status, '{$ct_status}') and mb_id = '{$member['mb_id']}' $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
...
$list_num = $total_cnt - ($page - 1) * $rows;
$result = sql_query(" select count(*) as cnt $sql_common ");
for ($i=0; $row=sql_fetch_array($result); $i++) { 
//    $list[i] = $row[i];
	// 주문정보
//	$list[$i] = sql_fetch(" select * from {$g5['g5_shop_order_table']} where find_in_set(od_status, '{$ct_status}') and mb_id = '{$member['mb_id']}' $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id");
	$list[$i]['num'] = $list_num;
...
//    $sql1 = " select a.od_id, a.ct_status, a.it_id, a.it_name, a.ct_send_cost, a.it_sc_type, a.pt_send, a.pt_send_num, b.pt_msg1, b.pt_msg2, b.pt_msg3
//			  from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id )
//			  where a.od_id = '{$row['od_id']}' and find_in_set(a.ct_status, '입금,준비') and find_in_set(a.pt_it, '{$ct_it}')=0 and a.pt_id = '{$member['mb_id']}' and a.ct_select = '1'
//			  group by a.it_id
//			  order by a.ct_id "
...
//$sql1 = " from {$g5['g5_shop_order_table']} where find_in_set(od_status, '주문') and mb_id = '{$member['mb_id']}' and $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
//
//$cnt1 = sql_query(" select count(*) as cnt1 $sql1 ");
//$total_1 = @sql_num_rows($cnt1); 
//
//$sql2 = " from {$g5['g5_shop_order_table']} where find_in_set(od_status, '입금') and mb_id = '{$member['mb_id']}' and $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
//
//$cnt2 = sql_query(" select count(*) as cnt2 $sql2 ");
//$total_2 = @sql_num_rows($cnt2); 
//
//
//$sql3 = " from {$g5['g5_shop_order_table']} where find_in_set(od_status, '준비') and mb_id = '{$member['mb_id']}' and $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
//
//$cnt3 = sql_query(" select count(*) as cnt3 $sql3 ");
//$total_3 = @sql_num_rows($cnt3); 
//
//$sql4 = " from {$g5['g5_shop_order_table']} where find_in_set(ct_status, '배송') and mb_id = '{$member['mb_id']}' and $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
//
//$cnt4 = sql_query(" select count(*) as cnt4 $sql4 ");
//$total_4 = @sql_num_rows($cnt4); 
//
//$sql5 = " from {$g5['g5_shop_order_table']} where find_in_set(ct_status, '완료') and mb_id = '{$member['mb_id']}' and $sql_search and SUBSTRING(od_time,1,10) between '$fr_day' and '$to_day' group by od_id ";
//
//$cnt5 = sql_query(" select count(*) as cnt5 $sql5 ");
//$total_5 = @sql_num_rows($cnt5); 

// 스킨
include_once($skin_path.'/searchorder.skin.php');
