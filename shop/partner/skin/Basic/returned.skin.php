               ...
               <?php if($j == 0) { ?>
				...
				<td>
					...
					<select class="form-control input-sm" onchange="sel_company('com_<?php echo $z;?>', this.value)">
						<?php echo get_delivery_company($list[$i]['it'][$j]['pt_receive'], '배송업체 선택');?>
					</select>
					<div style="height:8px;"></div>
					<input type="text" id="com_<?php echo $z;?>" name="pt_receive[<?php echo $z; ?>]" value="<?php echo $list[$i]['it'][$j]['pt_receive']; ?>" class="form-control input-sm">
				</td>
				<td>
					...
					<?php } ?>
					<input type="text" name="pt_receive_num[<?php echo $z; ?>]" value="<?php echo $list[$i]['it'][$j]['pt_receive_num']; ?>" class="form-control input-sm">
				</td>
				<td class="text-center">
				<?php 
				if($list[$i]['it'][$j]['ct_status'] == '반품접수'){ 

					$list[$i]['it'][$j]['ct_status'] = "반품접수";
				?>
					<!-- <input type="button" onclick="chkOne('<?=$list[$i]['it'][$j]['반품회수중'];?>');" class="btn btn-default" value="확인전"> -->
					<strong><font color="#ff6600"><? echo $list[$i]['it'][$j]['ct_status'];?></font></strong>

					<select name="ct_status[<?php echo $z; ?>]" class="form-control input-sm">
						<option value="반품접수"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'반품접수');?>>반품접수</option>
						<option value="반품회수중"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'반품회수중');?>>반품회수중</option>
						<!-- <option value="품절"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'품절');?>>품절</option> -->
					</select>

				<?
				}elseif($list[$i]['it'][$j]['ct_status'] == '반품회수중'){
					$list[$i]['it'][$j]['ct_status'] = "반품회수중";
				?>
				
				<strong><font color="#006600"><? echo $list[$i]['it'][$j]['ct_status'];?></font></strong>

				<?}else{?>

				<font color="#4f4f4f">
					<?if($list[$i]['it'][$j]['ct_status'] == '반품'){
						$list[$i]['it'][$j]['ct_status'] = "반품완료"; 
						echo $list[$i]['it'][$j]['ct_status'];
					}else{echo $list[$i]['it'][$j]['ct_status'];
					}?>
				</font>
...
<!-- 
					<select name="ct_status[<?php echo $z; ?>]" class="form-control input-sm">
						<option value="반품접수"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'반품접수');?>>반품접수</option>
						<option value="준비"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'반품회수중');?>>반품회수중</option>
						<option value="배송"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'배송');?>>배송완료</option>
					</select>
 -->
				</td>
			</tr>
	...
	<?php } ?>
	<?php if ($i == 0) { ?>
		<tr><td colspan="10" class="text-center">등록된 주문내역이 없습니다.</td></tr>
	<?php } ?>
	</tbody>
	</table>
</div>
...
<div style="margin-bottom:20px;">
	...
	<div class="form-group pull-right">			
		<button type="submit" class="btn btn-default">반품일괄처리</button>
	</div>
...
<form name="frmOne" method="post" action="처리할 url">
<input type="hidden" name="준비" >
...
<script>
	function chkOne(val)
	{
		...
		frm.<?$list[$i]['it'][$j]['반품회수중'];?>.value = val;
		frm.submit();
	}
</script>
