...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    // 옵션정보
    $sql2 = " select ct_option, ct_qty, io_price, pt_msg1, pt_msg2, pt_msg3
                from {$g5['g5_shop_cart_table']}
    ...
    for($k=0; $row2=sql_fetch_array($result2); $k++) {
        ...
        if($row2['io_price'] >= 0)
            $price_plus = '+';
        $options .= '<li'.$options_li.'>'.$row2['ct_option'].' ('.$price_plus.display_price($row2['io_price']).') '.$row2['ct_qty'].'개'.PHP_EOL;
		if($row2['pt_msg1'])
	        $options .= '<br>'.$row2['pt_msg1'];
		if($row2['pt_msg2'])
	        $options .= '<br>'.$row2['pt_msg2'];
		if($row2['pt_msg3'])
	        $options .= '<br>'.$row2['pt_msg3'];
        $options .= '</li>'.PHP_EOL;
	}
