...
if(!$board[bo_1]) $board[bo_1] = "10,20,30,40,50,70,100,200,300,500,1000,2000,3000,5000,10000";
$give_point = explode(",", $board[bo_1]);

if(!$write[wr_2]) $write[wr_2] = '5'; //답변대기로 기본입력
...
<style type="text/css">
.board_list tr:hover, .board_list tr:active {background-color:#fff;}
</style>
...
<div id="bo_w" class="write-wrap<?php echo (G5_IS_MOBILE) ? ' font-14' : '';?>">
	...
	<!-- 게시물 작성/수정 시작 { -->
	<form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
	...
	?>
		...
		<?php } ?>
       <div class="form-group">
			<label class="col-sm-2 control-label" for="wr_1">채택 포인트</label>
			<div class="col-sm-6">
				<select name="wr_1" itemname="채택 포인트"  required id="wr_1">
				    <option value=''>선택하세요.
						<?php for ($i=0; $i<count($give_point); $i++) { ?>
						<option value='<?php echo (int)$give_point[$i]?>' <?php if ($write[wr_1] == (int)$give_point[$i]) echo "selected"; ?>><?php echo (int)$give_point[$i]?>
						<?php } ?>
				</select> 
			</div>
		</div>
...
-->
				...
				<tr>
					...
					<td>
						<a href="javascript:win_calendar('wr_3', document.getElementById('wr_3').value, '');"><input class="field" style="width:60;" name="wr_3" id="wr_3" readonly itemname="답변 마감일" value="<?php echo $write[wr_3]?>"></a>
					</td>
				</tr>
				-->
