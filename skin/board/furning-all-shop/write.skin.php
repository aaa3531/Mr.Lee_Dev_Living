...
<div id="bo_w" class="write-wrap<?php echo (G5_IS_MOBILE) ? ' font-14' : '';?>">
    ...
    <?php
		...
		if ($is_notice || $is_html || $is_secret || $is_mail) {
			...
			if ($is_admin) {
				...
				$option .= "\n".'<label class="checkbox-inline"><input type="hidden" id="as_type" name="as_type" value="1" '.$main_checked.'></label>';
				$option_ctn++;
			}
	...
	<?php } ?>
    <div class="all-cat-bg">
      <div class="form-group all-cat">
			<label class="col-sm-2 control-label" for="wr_3">평수</label>
			<div class="col-sm-6">
				<select name="wr_3" required id="wr_3">
				    <option value="" selected>선택하세요</option>
				    <?php $arr = array("1-9평","10평대","20평대","30평대","40평대","50평대","60평대","70평대 이상");
                    foreach ($arr as $v){
		...
		<div class="form-group all-cat">
			<label class="col-sm-2 control-label" for="wr_4">예산</label>
			<div class="col-sm-6">
				<select name="wr_4" required id="wr_4">
				    <option value="" selected>선택하세요</option>
				    <?php $arr = array("천만원대 미만","1천만원대","2천만원대","3천만원대","4천만원대","5천만원대","6천만원대","7천만원대","8천만원대","9천만원대","1억원 이상");
                    foreach ($arr as $v){
		...
		<div class="form-group all-cat">
			<label class="col-sm-2 control-label" for="wr_5">컬러</label>
			<div class="col-sm-6">
				<select name="wr_5" required id="wr_5">
				    <option value="" selected>선택하세요</option>
				    <?php $arr = array("블랙","화이트","그레이","아이보리","베이지","브라운","레드","핑크","옐로우","그린","민트","블루","남색");
                    foreach ($arr as $v){
                        $selected = $write['wr_5'] == $v ? "selected" : "";
                        echo "<option value='".$v."'".$selected.">".$v."</option>";
		...
		<div class="form-group all-cat">
			<label class="col-sm-2 control-label" for="wr_6">분위기</label>
			<div class="col-sm-6">
				<select name="wr_6" required id="wr_6">
				    <option value="" selected>선택하세요</option>
				    <?php $arr = array("모던","클래식","레트로","우드","이색적인","따뜻함","차가움");
                    foreach ($arr as $v){
                        $selected = $write['wr_6'] == $v ? "selected" : "";
                        echo "<option value='".$v."'".$selected.">".$v."</option>";
		...
		<div class="form-group all-cat">
			<label class="col-sm-2 control-label" for="wr_7">업종</label>
			<div class="col-sm-6">
				<select name="wr_7" required id="wr_7">
				    <option value="" selected>선택하세요</option>
				    <?php $arr = array("카페","레스토랑","오피스","음식점","주방","숙박","병원","바","술집");
                    foreach ($arr as $v){
                        $selected = $write['wr_7'] == $v ? "selected" : "";
                        echo "<option value='".$v."'".$selected.">".$v."</option>";
	...
	<?php if ($is_category && $is_admin) { ?>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="ca_name">분류</label>
			<div class="col-sm-3">
...
-->
	...
	<?php if ($is_file) { ?>
		...
		<div class="form-group">
			<label class="col-sm-2 control-label">첨부파일(썸네일)</label>
			<div class="col-sm-10">
