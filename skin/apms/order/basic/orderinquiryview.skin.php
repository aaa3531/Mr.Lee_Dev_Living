	...
	<?php for($i=0; $i < count($item); $i++) { ?>
		<?php for($k=0; $k < count($item[$i]['opt']); $k++) { ?>
			...
			<tr>
				...
				<td><?php echo $item[$i]['it_name']; ?>/<?php echo $item[$i]['opt'][$k]['ct_option']; ?></td>
				...
				<?php if ($cancel_price != 0) { 
                       $retnurl = '/'; // 도메인 포함하여 입력
               $message = '[주문번호] '.$od_id.'\n[상품명] '.$item[$i]['it_name'].' ( 옵션: '.$item[$i]['opt'][$k]['ct_option'].')\n[주문자] '.get_text($od['od_name']).'\n주문이 취소되었습니다.';
               $kalabel = '주문취소'; // 알림받을시 표시될 제목
           
               include_once(G5_PLUGIN_PATH.'/wzkakao/config.php');
               $wzkakao = new wzkakao_mymsg();
               $wzkakao->first_send_msg($retnurl, $message, $kalabel);
                } else { 
               $retnurl = '/'; // 도메인 포함하여 입력
               $message = '[주문번호] '.$od_id.'\n[상품명] '.$item[$i]['it_name'].' ( 옵션: '.$item[$i]['opt'][$k]['ct_option'].')\n[주문자] '.get_text($od['od_name']).'\n[전화번호] '.get_text($od['od_b_tel']).'\n[배송지]  '.get_text(sprintf("(%s%s)", $od['od_b_zip1'], $od['od_b_zip2']).' '.print_address($od['od_b_addr1'], $od['od_b_addr2'], $od['od_b_addr3'], $od['od_b_addr_jibeon'])).'\n[주문일시] '.$od['od_time'].'\n[금액] '.display_price($tot_price);
               $kalabel = '주문알림'; // 알림받을시 표시될 제목
