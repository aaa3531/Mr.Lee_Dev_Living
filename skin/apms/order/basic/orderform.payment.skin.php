...
<!--		<div class="heading"><h3>결제정보</h3></div>-->
		<div class="panel-body">
			...
			<?php } else { ?>
			    ...
			    <div class="form-group">
					<label class="col-sm-2 control-label"><b>주문하기</b></label>
					<div class="col-sm-10 radio-line">
                        <label><input type="radio" id="od_settle_ohter" name="od_settle_case" value="기타결제"><p>렌탈결제</p></label>
					</div>
				</div>
			    <?php } else  {?>
				<div class="form-group">
					...
					<div class="col-sm-10 radio-line">
						...
						<?php if($is_mu) { ?>
							<label><input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장"> <p>무통장입금</p></label>
						<?php } ?>

						<?php if($is_vbank) { ?>
							<label><input type="radio" id="od_settle_vbank" name="od_settle_case" value="가상계좌"> <p><?php echo $escrow_title;?>가상계좌</p></label>
						<?php } ?>

						<?php if($is_iche) { ?>
                        <label><input type="radio" id="od_settle_iche" name="od_settle_case" value="계좌이체"> <p><?php echo $escrow_title;?>계좌이체</p></label>
						<?php } ?>

						<?php if($is_hp) { ?>
                        <label><input type="radio" id="od_settle_hp" name="od_settle_case" value="휴대폰"> <p>휴대폰</p></label>
						<?php } ?>

						<?php if($is_card) { ?>
                        <label><input type="radio" id="od_settle_card" name="od_settle_case" value="신용카드"> <p>신용카드</p></label>
						<?php } ?>

						<?php if($is_easy_pay) { ?>
                        <label><input type="radio" id="od_settle_easy_pay" name="od_settle_case" value="간편결제"> <p>K.PAY</p></label>
						<?php } ?>

						<?php if($is_kakaopay) { ?>
                        <label><input type="radio" id="od_settle_kakaopay" name="od_settle_case" value="KAKAOPAY"> <p><span class="kakaopay_icon">KAKAOPAY</span></p></label>
						<?php } ?>

						<?php if($is_samsung_pay) { ?>
							<label><input type="radio" id="od_settle_samsung_pay" data-case="samsungpay" name="od_settle_case" value="삼성페이"> <p><span class="samsung_pay">삼성페이</span></p></label>
						<?php } ?>

						<?php if($is_inicis_lpay) { ?>
                        <label><input type="radio" id="od_settle_inicislpay" data-case="lpay" name="od_settle_case" value="lpay"> <p><span class="inicis_lpay">L.pay</span></p></label>
						<?php } ?>
