...
<style>
	#cc-item .owl-container { margin-right:<?php echo $minus;?>px;}
	#cc-item .owl-next { right:<?php echo $gap;?>px; }
	#cc-item .item { margin-right:<?php echo $gap;?>px; }
	#cc-item .item-name { height:<?php echo $line_height;?>px; }
	#cc-item .img-wrap { padding-bottom:<?php echo $img_h;?>%; }
	#cc-item .owl-hide { margin-right:<?php echo ($item > 1) ? ($gap * ($item - 1)) : $gap;?>px; }
	#cc-item .owl-hide .item { width:<?php echo apms_img_width($item);?>%; } 
	<?php if(_RESPONSIVE_) { //반응형일 때만 작동 ?>
	@media (max-width:1199px) { 
		.responsive #cc-item .owl-hide { margin-right:<?php echo ($lg > 1) ? ($gap * ($lg - 1)) : $gap;?>px; }
		.responsive #cc-item .owl-hide .item { width:<?php echo apms_img_width($lg);?>%; } 
	}
	@media (max-width:991px) { 
		.responsive #cc-item .owl-hide { margin-right:<?php echo ($md > 1) ? ($gap * ($md - 1)) : $gap;?>px; }
		.responsive #cc-item .owl-hide .item { width:<?php echo apms_img_width($md);?>%; } 
	}
	@media (max-width:767px) { 
		.responsive #cc-item .owl-hide { margin-right:<?php echo ($sm > 1) ? ($gap * ($sm - 1)) : $gap;?>px; }
		.responsive #cc-item .owl-hide .item { width:<?php echo apms_img_width($sm);?>%; } 
	}
	@media (max-width:480px) { 
		.responsive #cc-item .owl-hide { margin-right:<?php echo ($xs > 1) ? ($gap * ($xs - 1)) : $gap;?>px; }
		.responsive #cc-item .owl-hide .item { width:<?php echo apms_img_width($xs);?>%; } 
	}
	<?php } ?>
</style>
<div id="cc-item" class="item_cc">
	<div class="owl-show">
...
$(document).ready(function(){
	$('#cc-item .owl-carousel').owlCarousel({
		...
		<?php } ?>
		pagination:true,
		<?php if(1 || isset($wset['nav']) && $wset['nav']) { ?> 
		...
		loop:true,
		afterInit : function() {
			$('#cc-item .owl-hide').hide();
		}
	});
});
</script>
<style>
#cc-item { overflow:hidden; line-height:20px; margin-bottom:30px; }
#cc-item .txt-normal { letter-spacing:0; }
#cc-item .txt-short { letter-spacing:-1px; padding-right:1px; }
#cc-item .item-row { float:left; }
#cc-item .item-image { position:relative; overflow:hidden; }
#cc-item .item-content { padding-top:10px; margin:0px 4px; text-align:center; }
#cc-item .item-star { text-align:center; overflow:hidden; letter-spacing:-1px; font-size:18px; line-height:20px; height:20px; margin-bottom:4px; }
#cc-item .item-name { overflow:hidden; letter-spacing:-1px; padding-right:1px; text-align:center; font-size:14px; }
#cc-item .item-text { font-size:12px; color:#888; }
#cc-item .item-price { margin-top:4px; line-height:22px; }
#cc-item .item-price b { font-size:16px; display:inline-block; margin:0px 4px; }
#cc-item .item-price b i { font-size:12px; }
#cc-item .item-price strike { color:#888; margin:0px 4px; font-size:14px; display:inline-block; }
#cc-item .item-details { color:#888; margin-top:4px; padding:0px 1px; font-size:13px; text-align:center; }
#cc-item .item-sp { margin:0px 4px; white-space:nowrap; }
#cc-item .item-sp i { margin-right:2px; color:#888; }
#cc-item .item-sns { margin-top:15px; text-align:center; }
#cc-item .item-sns img { width:15%; max-width:34px; border-radius:50%; }

.item_cc .owl-container{
	padding-bottom:40px;
}
.item_cc .owl-pagination{
	bottom:-20px;
}
.item_cc .owl-controls .owl-page span{
	background: rgba(128, 128, 128, 0.4);
}

.item_cc .owl-controls .owl-page.active span, .item_cc .owl-controls.clickable .owl-page:hover span{
	background: rgba(50, 60, 70, 0.75) !important;
}

.item_cc .owl-carousel .owl-controls.clickable .owl-buttons div{
	display:block;
}

.item_cc .owl-carousel .owl-controls .owl-buttons div{
	margin-top: -55px;
}
</style>