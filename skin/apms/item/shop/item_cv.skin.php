...
<style>
	#cv-item .owl-container { margin-right:<?php echo $minus;?>px;}
	#cv-item .owl-next { right:<?php echo $gap;?>px; }
	#cv-item .item { margin-right:<?php echo $gap;?>px; }
	#cv-item .item-name { height:<?php echo $line_height;?>px; }
	#cv-item .img-wrap { padding-bottom:<?php echo $img_h;?>%; }
	#cv-item .owl-hide { margin-right:<?php echo ($item > 1) ? ($gap * ($item - 1)) : $gap;?>px; }
	#cv-item .owl-hide .item { width:<?php echo apms_img_width($item);?>%; } 
	<?php if(_RESPONSIVE_) { //반응형일 때만 작동 ?>
	@media (max-width:1199px) { 
		.responsive #cv-item .owl-hide { margin-right:<?php echo ($lg > 1) ? ($gap * ($lg - 1)) : $gap;?>px; }
		.responsive #cv-item .owl-hide .item { width:<?php echo apms_img_width($lg);?>%; } 
	}
	@media (max-width:991px) { 
		.responsive #cv-item .owl-hide { margin-right:<?php echo ($md > 1) ? ($gap * ($md - 1)) : $gap;?>px; }
		.responsive #cv-item .owl-hide .item { width:<?php echo apms_img_width($md);?>%; } 
	}
	@media (max-width:767px) { 
		.responsive #cv-item .owl-hide { margin-right:<?php echo ($sm > 1) ? ($gap * ($sm - 1)) : $gap;?>px; }
		.responsive #cv-item .owl-hide .item { width:<?php echo apms_img_width($sm);?>%; } 
	}
	@media (max-width:480px) { 
		.responsive #cv-item .owl-hide { margin-right:<?php echo ($xs > 1) ? ($gap * ($xs - 1)) : $gap;?>px; }
		.responsive #cv-item .owl-hide .item { width:<?php echo apms_img_width($xs);?>%; } 
	}
	<?php } ?>
</style>
<div id="cv-item" class="item_cc">
	<div class="owl-show">
...
$(document).ready(function(){
	$('#cv-item .owl-carousel').owlCarousel({
		...
		loop:true,
		afterInit : function() {
			$('#cv-item .owl-hide').hide();
		}
	});
});
</script>
<style>
#cv-item { overflow:hidden; line-height:20px; margin-bottom:30px; }
#cv-item .txt-normal { letter-spacing:0; }
#cv-item .txt-short { letter-spacing:-1px; padding-right:1px; }
#cv-item .item-row { float:left; }
#cv-item .item-image { position:relative; overflow:hidden; }
#cv-item .item-content { padding-top:10px; margin:0px 4px; text-align:center; }
#cv-item .item-star { text-align:center; overflow:hidden; letter-spacing:-1px; font-size:18px; line-height:20px; height:20px; margin-bottom:4px; }
#cv-item .item-name { overflow:hidden; letter-spacing:-1px; padding-right:1px; text-align:center; font-size:14px; }
#cv-item .item-text { font-size:12px; color:#888; }
#cv-item .item-price { margin-top:4px; line-height:22px; }
#cv-item .item-price b { font-size:16px; display:inline-block; margin:0px 4px; }
#cv-item .item-price b i { font-size:12px; }
#cv-item .item-price strike { color:#888; margin:0px 4px; font-size:14px; display:inline-block; }
#cv-item .item-details { color:#888; margin-top:4px; padding:0px 1px; font-size:13px; text-align:center; }
#cv-item .item-sp { margin:0px 4px; white-space:nowrap; }
#cv-item .item-sp i { margin-right:2px; color:#888; }
#cv-item .item-sns { margin-top:15px; text-align:center; }
#cv-item .item-sns img { width:15%; max-width:34px; border-radius:50%; }
