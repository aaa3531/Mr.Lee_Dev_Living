...
if ($i==0) 
    ...
    echo '<div class="banner-img">'.PHP_EOL;
    ...
    if (file_exists($bimg))
    {
        ...
        if ($row['bn_url'][0] == '#')
            ...
            $banner .= '<a class="sv" href="'.G5_SHOP_URL.'/bannerhit.php?bn_id='.$row['bn_id'].'"'.$bn_new_win.'>';
        }
...
if ($i > 0) {
    ...
    <ul>';
		...
		foreach( $main_banners as $row ){
			echo '<li> <span onclick="currentSlide('.$k.')" class="link-banner" data-slide-index="'.$k.'" >'.get_text($row['bn_alt']).'</span></li>'.PHP_EOL;
			$k++;
		}
    echo '</ul>
    </div>'.PHP_EOL;
