...
?>
<h2 class="search-tit"><b>'<?php echo $text_stx ?>'</b>게시물 검색결과</h2>
<p class="search-info">전체 <b><?php echo number_format($total_count); ?></b> 개의 게시물이 검색되었습니다.</p>
...
if ($stx) {
 ...
 ?>
    <div class="search-board">
        <ul>
            <li><a href="?<?php echo $search_query ?>&amp;gr_id=<?php echo $gr_id ?>">전체게시판 (<?php echo number_format($total_count); ?>)</a></li>
           <?php for($i=0;$i < count($bo_list);$i++) { ?>
            <li><a href="<?php echo $bo_list[$i]['href']; ?>"><?php echo $bo_list[$i]['name'];?> (<?php echo number_format($bo_list[$i]['cnt']);?>)</a></li>
           <?php } ?>
        </ul>
    </div>
	<br>
