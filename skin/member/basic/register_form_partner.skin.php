...
<form class="form-box form-horizontal register-form" role="form" id="fregisterform" name="fregisterform" action="<?php echo $action_url ?>" onsubmit="return fregisterform_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
...
<h3>파트너 회원가입</h3>
	<div class="panel-join-member">
		<div class="panel-body">
            <input type="text" name="mb_id" value="<?php echo $member['mb_id'] ?>" id="reg_mb_id" <?php echo $required ?> <?php echo $readonly ?> class="input-join" minlength="3" maxlength="20" placeholder="아이디 입력">
            <p>영문자, 숫자, _ 만 입력 가능. 최소 3자이상 입력하세요.</p>
            <input type="password" name="mb_password" id="reg_mb_password" <?php echo $required ?> class="input-join" minlength="3" maxlength="20" placeholder="비밀번호">
            <input type="password" name="mb_password_re" id="reg_mb_password_re" <?php echo $required ?> class="input-join" minlength="3" maxlength="20" placeholder="비밀번호 확인">
			<hr>
			<input type="text" id="reg_mb_name" name="mb_name" value="<?php echo get_text($member['mb_name']) ?>" <?php echo $required ?> <?php echo $readonly; ?> class="input-join" size="10" placeholder="파트너사명 입력">
					...
					<div class="col-sm-offset-2 col-sm-8 text-muted">
						...
						?>
							<span class="black" id="msg_certify">
								[<strong><?php echo $mb_cert; ?> 본인확인</strong><?php if ($member['mb_adult']) { ?> 및 <strong>인증하기</strong><?php } ?> 완료]
							</span>
						<?php } ?>
			...
			<?php } ?>
			<?php if ($req_nick) {  ?>
			<p>공백없이 한글,영문,숫자만 입력 가능 (한글2자, 영문4자 이상) 닉네임을 바꾸시면 앞으로 <?php echo (int)$config['cf_nick_modify'] ?>일 이내에는 변경 할 수 없습니다.</p>
			<input type="hidden" name="mb_nick_default" value="<?php echo isset($member['mb_nick']) ? get_text($member['mb_nick']) : ''; ?>">
              <input type="text" name="mb_nick" value="<?php echo isset($member['mb_nick']) ? get_text($member['mb_nick']) : ''; ?>" id="reg_mb_nick" required class="input-join nospace" size="10" maxlength="20" placeholder="닉네임 입력">
			<?php }  ?>
			<?php if ($config['cf_use_email_certify']) {  ?>
			<p><?php if ($w=='') { echo "E-mail 로 발송된 내용을 확인한 후 인증하셔야 회원가입이 완료됩니다."; }  ?>
						<?php if ($w=='u') { echo "E-mail 주소를 변경하시면 다시 인증하셔야 합니다."; }  ?></p>
			<?php }  ?>
			<input type="hidden" name="old_email" value="<?php echo $member['mb_email'] ?>">
             <input type="text" name="mb_email" value="<?php echo isset($member['mb_email'])?$member['mb_email']:''; ?>" id="reg_mb_email" required class="input-join email" size="70" maxlength="100" placeholder="이메일">
             <?php if ($config['cf_use_hp'] || $config['cf_cert_hp']) {  ?>
						<input type="text" name="mb_hp" value="<?php echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" <?php echo ($config['cf_req_hp'])?"required":""; ?> class="input-join" maxlength="20" placeholder="휴대폰 번호 입력 (- 제외)">
	...
	<div class="panel panel-default">
		<div class="panel-body">
			...
			<?php if ($config['cf_use_tel']) {  ?>
				<div class="form-group has-feedback">
					<label class="col-sm-2 control-label" for="reg_mb_tel"><b>대표번호</b><?php if ($config['cf_req_tel']) { ?><strong class="sound_only">필수</strong><?php } ?></label>
					<div class="col-sm-3">
	...
	<div class="panel-join-member">
		<div class="panel-body">
			...
			<?php if ($config['cf_use_signature']) {  ?>
				<div class="form-group">
					<label class="col-sm-2 control-label" for="reg_mb_signature"><b>대표자명</b><?php if ($config['cf_req_signature']){ ?><strong class="sound_only">필수</strong><?php } ?></label>
					<div class="col-sm-8">
	...
	<div class="join-button-gr">
		<button type="submit" id="btn_submit" class="join-button" accesskey="s"><?php echo $w==''?'파트너 가입':'정보수정'; ?></button>
		<?php if(!$pim) { ?>
			<a href="<?php echo G5_URL ?>" class="join-button" role="button">가입취소</a>
		<?php } ?>
	</div>
