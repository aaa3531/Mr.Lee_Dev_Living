...
<!--           <span class="rank-index"><b>BEST</b><?php echo $rank;$rank++;?></span>-->
           <div class="thumb">
            ...
            <h5><?php if($list[$i]['it_tel_inq']) { ?>Call<?php } else { ?><?php echo number_format($list[$i]['it_price']);?><?php } ?><?php if($dc_per) { ?><span class="dc">| <?php echo $dc_per;?>%</span><?php } ?> 원</h5>
            <!--<h6><span>판매자 |</span> <?php if($list[$i]['pt_id']) { //파트너아이디가 있으면...
                                         $mb = get_member($list[$i]['pt_id']);
                                         echo $mb['mb_nick']; //회원닉네임
    } ?></h6>
<!--            <p><span class="star"><i class="fa fa-star fa-lg red"></i><?php echo $list[$i]['it_use_avg'] ?>점 | <?php echo $list[$i]['it_use_cnt'] ?> 명 참여</span></p>--></div>
       </a>
    </li>
    <?php } ?>
</ul>
