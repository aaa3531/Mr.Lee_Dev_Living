...
if ($_POST['act_button'] == "선택삭제") {
    ...
    for ($i=0; $i<count($_POST['chk']); $i++) {
		...
		// 자료 불러오기
		$row = sql_fetch(" select it_id, iq_question, iq_answer from {$g5['g5_shop_item_qa_table']} where iq_id = '{$iiq_id}' ");
		...
		// 삭제
        sql_query(" delete from {$g5['g5_shop_item_qa_table']} where iq_id = '{$iiq_id}' ");
