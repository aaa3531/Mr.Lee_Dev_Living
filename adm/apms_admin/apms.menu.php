...
<style>
.tbl_head01 tbody td.td-inline { text-align:left; }
...
<div class="local_desc01 local_desc">
	<?php if(IS_YC) { ?>
		보드그룹 또는 상품분류별로 테마 미설정시 각 보드그룹은 커뮤니티 기본 테마가 각 상품분류는 쇼핑몰 기본 테마가 자동 적용됩니다.
	<?php } else { ?>
		보드그룹별로 테마 미설정시 각 보드그룹은 커뮤니티 기본 테마가 자동 적용됩니다.
	<?php } ?>
	<br>
	접근레벨은 최소레벨(위)과 최대레벨(아래)을 각각 입력하시면 됩니다. 단, 관리자는 접근등급과 레벨이 적용안됩니다.
</div>
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
	<td>새글 표시 시간</td>
	<td><input type="text" name="cf_10" size="4" value="<?php echo ((int)$config['cf_10'] >= 0) ? $config['cf_10'] : 24; ?>" class="frm_input"></td>
	<td class="td_left">시간 이내 등록된 자료가 있는 메뉴는 새글 표시('0' 입력시 새글표시 안함)</td>
	</tr>
	<tr>
	<td>자동 메뉴 캐시</td>
	<td><input type="text" name="cf_9" size="4" value="<?php echo ((int)$config['cf_9'] >= 0) ? $config['cf_9'] : 0; ?>" class="frm_input"></td>
	<td class="td_left">초 단위로 자동 메뉴 새로고침('0' 입력시 실시간 출력 - 느림)</td>
	</tr>
	<tr>
	<td>통계 현황 캐시</td>
	<td><input type="text" name="cf_10_subj" size="4" value="<?php echo ((int)$config['cf_10_subj'] >= 0) ? $config['cf_10_subj'] : 0; ?>" class="frm_input"></td>
	<td class="td_left">초 단위로 통계를 새로고침('0' 입력시 실시간 출력 - 느림)</td>
	</tr>
	</tbody>
	</table>
</div>
...
<div class="local_desc01 local_desc">
	<b>● 보드그룹메뉴 설정</b> - 메뉴보임이 설정된 그룹과 게시판만 출력됩니다.
</div>
...
<div class="tbl_head01 tbl_wrap">
	...
	?>
		<tr>
		<td align="center"><a href="./apms.groupmenu.php?gr_id=<?php echo $row['gr_id'];?>" class="btn btn_03 win_point"><nobr>서브메뉴설정</nobr></a></td>
		<td align="center">
			<select name="as_show[<?php echo $z; ?>]">
				<option value="0">숨김메뉴</option>
		...
		<td align=center>
			<a href="<?php echo G5_BBS_URL;?>/main.php?gid=<?php echo $row['gr_id'];?>" target="_blank"><?php echo $row['gr_id']; ?></a>
		</td>
		<td>
		...
		<td>
			...
			<div id="bbs_<?php echo $i;?>" style="display:none;width:290px;">
				<table>
				<tr>
				<th width="140">PC 그룹스킨</th>
				<td>
...
<?php if(IS_YC) { ?>
	...
	<div class="local_desc01 local_desc">
		<b>● 상품분류메뉴 설정</b> - 메뉴보임이 설정된 상품분류만 출력됩니다.
	</div>
	...
	<div class="tbl_head01 tbl_wrap">
		...
		<th width=120><nobr>분류메뉴</nobr></th>
		<th width=50><nobr>순서</nobr></th>
		<th width=80><nobr>분류코드</nobr></th>
		...
		?>
			...
			<?php if($sub['cnt']) { ?>
				<td width="90" align="center">
					<a href="./apms.shopmenu.php?cid=<?php echo $row['ca_id'];?>" class="btn btn_03 win_point"><nobr>서브메뉴설정
					</nobr></a>
				</td>
			...
			<td>
				...
				<div id="shop_<?php echo $i;?>" style="display:none;width:290px;">
					<table>
					<tr>
					<th width="140">별도분류링크</th>
					<td>
...
<div class="btn_fixed_top">
	...
	<!--
    <a href="<?php echo G5_ADMIN_URL;?>/boardgroup_list.php" class="btn btn_02">보드그룹 관리</a>
	<?php if (IS_YC) { ?>
		<a href="<?php echo G5_ADMIN_URL;?>/shop_admin/categorylist.php" class="btn btn_02">상품분류 관리</a>
	<?php } ?>
	-->
