...
if($save) {
    ...
    for ($i=0; $i<count($_POST['od_id']); $i++) {
		...
		if($_POST['pt_receive'][$i] && $_POST['pt_receive_num'][$i]) {
			$ct_status = '반품';
		} else if($_POST['ct_status'][$i] == '반품접수' || $_POST['ct_status'][$i] == '반품회수중' || $_POST['ct_status'][$i] == '반품') {
        ...
        $sql = "update {$g5['g5_shop_cart_table']}
                   set pt_receive        = '".addslashes($_POST['pt_receive'][$i])."',
                       pt_receive_num    = '".addslashes($_POST['pt_receive_num'][$i])."',
                       ct_status	  = '{$ct_status}'
...
$fr_date = ($fr_date) ? $fr_date : date("Ymd", G5_SERVER_TIME - 30*86400); //보름전
...
// 상태
if(isset($done) && $done) { 
	$ct_status = ($done == "2") ? '반품' : '반품접수,반품회수중'; 
} else { 
	$ct_status = '반품접수,반품회수중,반품'; 
} 
...
for ($i=0; $row=sql_fetch_array($result); $i++) { 
	...
	// 주문상품 - 파트너 아이디로도 체크
	$sql1 = " select a.od_id, a.pt_id, a.ct_status, a.it_id, a.it_name, a.ct_send_cost, a.it_sc_type, a.pt_receive, a.pt_receive_num, b.pt_msg1, b.pt_msg2, b.pt_msg3
	...
	for($j=0; $row1=sql_fetch_array($result1); $j++) {
		...
		// 문의전화
		$list[$i]['it'][$j]['tel'] = get_delivery_inquiry($row1['pt_receive'], $row1['pt_receive_num']);
	}
...
<div class="local_ov01 local_ov">
	반품가능한 상품의 주문건 중 반품접수/반품회수중/반품상태의 주문만 출력됩니다. 미반품건은 반품접수/반품회수중상태, 반품완료는 반품중인 주문건을 말합니다.
</div>
...
<div class="tbl_head01 tbl_wrap">
	<ul style="background: rgb(249, 249, 249); padding: 10px 30px; border: 1px solid rgb(242, 242, 242); line-height: 20px; margin-bottom: 10px;">
		<li>총 <b><?php echo number_format($total_count); ?></b>건의 
			<?php
				switch($done) {
					case '1'	: echo '미반품'; break;
					case '2'	: echo '반품완료된'; break;
				}
			?>
			주문이 있습니다.
		</li>
			...
			<tr class="od-tr">
               <?php if($j == 0) { ?>
				...
				<td align="center">
					...
					<input type="text" id="com_<?php echo $z;?>" name="pt_receive[<?php echo $z; ?>]" value="<?php echo $list[$i]['it'][$j]['pt_receive']; ?>" class="frm_input od-send">
				</td>
				<td align="center">
					<?php if($list[$i]['it'][$j]['tel']) { ?>
						...
						<div style="height:6px;"></div>
					<?php } ?>
					<input type="text" name="pt_receive_num[<?php echo $z; ?>]" value="<?php echo $list[$i]['it'][$j]['pt_receive_num']; ?>" class="frm_input od-send">
				</td>
				<td align="center">
					<select name="ct_status[<?php echo $z; ?>]">
						...
						<option value="반품"<?php echo get_selected($list[$i]['it'][$j]['ct_status'],'반품');?>>반품완료</option>
					</select>
				</td>
			</tr>
		<?php $z++; } ?>
...
<div class="btn_confirm01 btn_confirm" style="float:right">
	<input type="submit" value="반품일괄처리" class="btn_submit">
</div>
